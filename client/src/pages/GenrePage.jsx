...
const GenrePage = () => {
    ...
    const genreName = location.state?.genreName || 'Genre'; // Fallback if state not passed
    ...
    useEffect(() => {
        const loadInitialData = async () => {
            ...
            setItems([]);
            try {
                const data = await api.getMoviesByGenre(id);
    ...
    const handleLoadMore = async () => {
        ...
        try {
            const data = await api.getMoviesByGenre(id, nextPage);
        ...
        <div className="container fade-in" style={{ paddingTop: '2rem', paddingBottom: '2rem' }}>
            ...
            <div style={{
                ...
                {items.map((movie, index) => (
                    <Link to={`/movie/${movie.id}`} key={`${movie.id}-${index}`} style={{ display: 'block' }}>
                        <div style={{
                        ...
                        >
                            ...
                            <div style={{ padding: '1rem' }}>
                                <h3 style={{ fontSize: '0.9rem', marginBottom: '0.5rem', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{movie.title}</h3>
